...
searchContainer.innerHTML = `<input type="text" id="searchNama" placeholder="Cari nama peserta..." style="padding:8px 12px;border-radius:8px;width:70%;max-width:300px;border:1px solid var(--text-color);outline:none;background:var(--input-bg);color:var(--text-color);text-align:center;transition:0.3s;">`;  
...
<br><b>Poin Juri:</b><br>  
💡 Kreativitas:<br>• ide konsep (150),<br>• editing (100),<br>• karakter (50).<br>  
...
function hitungTotal(p, tampilkanPoin) {  
  ...
  const total = tampilkanPoin ? +(nilaiKreatif + nilaiLokal + viral).toFixed(1) : +(nilaiKreatif + nilaiLokal).toFixed(1);  
...
function tampilkanDataSeason() {  
  ...
  const isDark = document.body.classList.contains("dark-theme");  
  infoRange.innerHTML = `<div style="background:${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)'};padding:10px;border-radius:10px;margin-top:8px;"><div style="font-weight:700;color:${isDark ? '#fff':'#222'};">${awal} - ${akhir}</div><div style="margin-top:4px;font-size:0.95em;"><span style="color:${isDark ? '#ffeb3b':'#b8860b'};">🎗️ Sponsor:</span><br><span style="font-style:italic;color:${isDark ? '#fdd835':'#5a4b00'};">${sponsor}</span></div></div>`;  
  ...
  for (let i = 1; i <= totalPages; i++) {  
    ...
    btn.onclick = () => { currentPage = i; tampilkanDataSeason(); document.getElementById("poin").scrollIntoView({ behavior: "smooth" }); };  
  ...
  (dataSeason.Hadiah || []).forEach(h => {  
    ...
    card.style.cssText = `background:var(--card-bg);color:var(--text-color);border-radius:12px;padding:15px;margin:10px;box-shadow:var(--shadow);text-align:center;transition:0.3s;`;  
