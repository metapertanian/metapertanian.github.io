...
searchContainer.innerHTML =   <input type="text" id="searchNama" placeholder="Cari nama kreator..."    style="padding:8px 12px;border-radius:8px;width:70%;max-width:300px;   border:1px solid var(--text-color);outline:none;   background:var(--input-bg);color:var(--text-color);   text-align:center;transition:0.3s;">      ;
...
// 🎨 Info Season
infoRange.innerHTML =    <div style="background:${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)'};padding:12px;border-radius:12px;margin-top:8px;">   <div style="font-size:1.1em;font-weight:700;color:${isDark ? '#ffeb3b' : '#b8860b'};">   🎬 Tema: <span style="color:${isDark ? '#fff' : '#111'};">${dataSeason.tema || "Tanpa Tema"}</span>   </div>   <div style="font-size:0.95em;margin-top:6px;line-height:1.4;">${dataSeason.deskripsi || ""}</div><br>   <div style="margin-top:6px;color:${isDark ? '#bbb' : '#333'};">📅 ${dataSeason.periode || "-"}</div><br>   <div style="margin-top:4px;font-size:0.95em;">   <span style="color:${isDark ? '#ffeb3b':'#b8860b'};">🎗️ Sponsor:</span><br>   <span style="font-style:italic;color:${isDark ? '#fdd835':'#5a4b00'};">${dataSeason.Sponsor || "-"}</span>   </div>   </div>;

// 📜 Aturan Lomba
document.getElementById("aturanText").innerHTML =   • Lomba terbuka untuk umum.<br>   • Konten sesuai tema: <b>${dataSeason.tema}</b><br>   • ${dataSeason.deskripsi}<br><br>   • Video hasil karya sendiri dan belum pernah diunggah.<br>   • Gaya bebas: lucu, edukatif, dokumenter, cinematic, atau motivasi.<br><br>   <b>Poin Juri:</b><br>   💡 Kreativitas (maks 300):<br>   ide konsep (150), <br>   editing (100), <br>   karakter (50).<br><br>   🏡 Lokal (maks 200):<br>   nuansa lokal (100), <br>   dampak positif (100).<br><br>   <b>Poin TikTok:</b><br>   🚀 Viral: poin tak terbatas,<br> dihitung dari like, komen, share.<br><br>   • Diperbolehkan mengajak teman atau saudara untuk menaikkan like, komen, share.<br>   • Dilarang spam/bot/beli untuk menaikkan like, komen, share.<br>   • Pelanggaran akan dikurangi poin atau bahkan diskualifikasi.  ;
...
div.style.cssText =   background:var(--card-bg);   border-radius:12px;   box-shadow:var(--shadow);   padding:12px;   margin:10px 0;   transition:transform .2s;  ;
...
btn.textContent = i;
btn.style.cssText =   margin:3px;   padding:6px 10px;   border-radius:8px;   border:none;   cursor:pointer;   background:${i === currentPage ? 'var(--highlight)' : (isDark ? '#333' : '#ddd')};   color:${i === currentPage ? '#000' : (isDark ? '#fff' : '#111')};   transition:all .2s;  ;
